...
let () =
  ...
  if mode = "correctness" then begin
    ...
    Printf.printf
      "RESULT benchmark=SBPF-instruction metric=%s process_id=%d cases=%d suite_repetitions=1 logical_units=%d elapsed_seconds=%.9f allocated_bytes=%.0f\n"
      mode (Unix.getpid ()) (List.length cases) (List.length cases) elapsed allocated
  end
