...
text \<open>
  ...
  The three shadowed let-bindings of \<open>x\<close> form a handoff chain
  (\<open>let x = n in let x = S x in let x = S x in x\<close>) that Thingol prints as the
  distinct variables \<open>x\<close>, \<open>xa\<close>, \<open>xb\<close> joined by \<open>.clone()\<close> hand-offs.  The mut
  pass collapses that chain into a single \<open>let mut x\<close> updated by assignment and
  drops the now-redundant handoff clones.
...
definition grow :: "peano \<Rightarrow> peano" where
  "grow n = (let x = n in let x = S x in let x = S x in x)"
