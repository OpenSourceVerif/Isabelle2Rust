...
lemma "k \<notin> set |q| \<Longrightarrow> priority (push k p q) k = Some p"
  apply(simp add:priority_def)
  by (metis alist_of_push fst_conv insertCI priority_def priority_fst set_insort_key snd_conv) 

lemma "k \<in> set |q| \<Longrightarrow> priority (push k p q) k = priority q k"
...
lemma "\<lbrakk> \<not> is_empty q; v \<in> set |q| \<rbrakk>
   \<Longrightarrow> the (priority q (min q)) \<le> the (priority q v)"
  apply(simp add:min_def priority_def is_empty_def values_def) 
  apply auto
  apply(cases "alist_of q") apply simp
  apply simp 
  using alist_of[of q] mem_Collect_eq apply simp 
  apply(case_tac "(v, b) = a") apply auto[1]
  apply simp
  by (metis snd_conv)

lemma "pop q = Some (v, q1) \<Longrightarrow> v = min q"
  apply(simp add:pop_def) apply(cases "is_empty q")
    apply simp
  by(simp add:min_def is_empty_def remove_min_def) 

lemma "pop q = Some (v, q1) \<Longrightarrow> v \<notin> set |q1|"
  apply(simp add:pop_def) apply(cases "is_empty q")
    apply simp
  apply(simp add:min_def is_empty_def remove_min_def values_def) 
  apply(cases "alist_of q") apply simp 
  using alist_of_Abs_pq alist_of[of q] mem_Collect_eq apply simp
  apply(subgoal_tac "sorted (map snd list)") 
    prefer 2 apply simp
  apply(subgoal_tac "alist_of (Abs_pq list) = list")
    by auto
...
no_notation
  "PriorityQueue_Test.values" ("|(_)|")
  and "PriorityQueue_Test.priorities" ("\<parallel>(_)\<parallel>")

value "PriorityQueue_Test.values (PriorityQueue_Test.push (4::int) (0::nat) PriorityQueue_Test.empty)"


export_code PriorityQueue_Test.values PriorityQueue_Test.priorities PriorityQueue_Test.is_empty PriorityQueue_Test.priority PriorityQueue_Test.min PriorityQueue_Test.empty PriorityQueue_Test.push PriorityQueue_Test.remove_min PriorityQueue_Test.pop in Rust
