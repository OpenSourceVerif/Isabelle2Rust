...
type instruction = Pmovl_rr of ireg * ireg | Pmovq_rr of ireg * ireg |
  ...
  Pcall_i of num1 bit0 bit0 bit0 bit0 bit0 word | Pret | Prdtsc | Pnop | P;;
...
let rec x64_encode
 ...
 Some ([rex; of_int (len_bit0 (len_bit0 (len_bit0 len_num1)))
   ...
   (Bit0 (Bit0 (Bit0 (Bit0 One))))))))
          ...
          | Prorl_ri (_, _) -> None | Prorq_ri (_, _) -> None
          | Pbswapl rd ->
            (let rex =
               construct_rex_to_u8 false false false
                 (not (equal_word (len_bit0 (len_bit0 (len_bit0 len_num1)))
...
let parse_instruction (line : string) : instruction option =
  ...
  | opc :: rest ->            
    ...
    | ("Pmovl_ri" | "Paddl_ri" | "Psubl_ri" | "Pandl_ri" | "Porl_ri" | "Pxorl_ri"), [r1; imm32] ->
        let* rd   = parse_ireg r1       in
    ...
    | _, [r1; r2] when
        List.mem opc
          ["Paddq_rr";"Psubq_rr";"Porq_rr";"Pandq_rr";"Pxorq_rr";
           "Pmovq_rr";"Pxchgq_rr";"Pmovsl_rr";"Paddl_rr";"Psubl_rr";
           "Porl_rr"; "Pandl_rr"; "Pxorl_rr";"Pmovl_rr"] ->
        let* rd = parse_ireg r1 in
        let* rs = parse_ireg r2 in
        begin match opc with
    ...
    | _, [r1] when
        List.mem opc
          ["Pmulq_r";"Pimulq_r"; "Pmull_r";"Pimull_r";
           ...
           "Pshlq_r";"Pshrq_r";"Psarq_r";
           "Pshll_r";"Pshrl_r";"Psarl_r";
           "Pnegq";  "Pnegl"] ->
        let* rd = parse_ireg r1 in
        begin match opc with
...
(*
  let parse_instruction1 (line : string) : instruction option =
  match split_ws line with
  | [opc; r1; r2_or_imm] -> begin
      let* rd = parse_ireg r1 in
      match parse_ireg r2_or_imm with
      | Some rs -> 
          begin match opc with
          | "Paddq_rr" -> Some (Paddq_rr (rd, rs))
          | "Psubq_rr" -> Some (Psubq_rr (rd, rs))
      ...
      | None ->
          let* imm8 = parse_u8_word r2_or_imm in
          let* imm32 = parse_u32_word r2_or_imm in
          begin match opc with
          | "Pmovl_ri" -> Some (Pmovl_rr (rd, imm32))
          | "Pshlq_ri" -> Some (Pshlq_ri (rd, imm8))
          | "Pshrq_ri" -> Some (Pshrq_ri (rd, imm8))
          | "Psarq_ri" -> Some (Psarq_ri (rd, imm8))
          | "Prorq_ri" -> Some (Psarq_ri (rd, imm8))
          | _ -> None
          end
    end
