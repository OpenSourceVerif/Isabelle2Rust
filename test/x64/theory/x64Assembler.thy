...
"x64_encode ins = (
  ...
  Pmovq_ri rd n \<Rightarrow>
    ...
    let (op::u8) = bitfield_insert 0 3 0xb8 (u8_of_ireg rd) in
  ...
  Pcmovl t rd r1 \<Rightarrow>
    ...
    let (ex::u8) = 0x0f in
    let (op::u8) = bitfield_insert 0 4 0x40 (u8_of_cond t) in   \<comment> \<open> 45 : NZ/NE （ZF=0 \<close>   
    let (rop::u8) = construct_modsib_to_u8 0b11 (u8_of_ireg rd) (u8_of_ireg r1) in
    if rex = 0x40 then
      Some [ex, op, rop]
    else
 ...
 \<comment> \<open> P2891 `AND: immediate to register` -> `0100 000B : 1000 00sw : 11 100 reg : immediate` \<close>
  ...
  Prorl_ri rd n \<Rightarrow>
    ...
    let (imm::u8) = ucast n in
      if rex = 0x40 then
        ...
        Some [ rex, op, rop, imm ] |
  \<comment> \<open> P2888 `ROR qwordregister by immediate count` -> ` 0100 100B 1100 0001 : 11 001 qwordreg : imm8 ` \<close>
  Prorq_ri rd n \<Rightarrow>
    let (rex:: u8) = (construct_rex_to_u8  \<comment> \<open> `100B` \<close>
      True \<comment> \<open> W \<close>
      False \<comment> \<open> R \<close>
      False \<comment> \<open> X \<close>
  ...
  Pbswapl rd  \<Rightarrow>
    ...
    let (ex::u8) = 0x0f in
    let (op::u8) = bitfield_insert 0 3 0xc8 (u8_of_ireg rd) in
      if rex = 0x40 then
        Some [ex, op]
      else 
        Some [rex, ex, op] |
  ...
  \<comment> \<open> P2881 `LEA: Load Effective Address: in qwordregister `  -> `0100 1RXB : 1000 1101 : mod qwordreg r/m` \<close>
   \<comment> \<open>we currently ignore the fact that rsp and r12 require sib encoding \<close>
  ...
  Ppushl_r  r1 \<Rightarrow>
    ...
    let (op::u8) = bitfield_insert 0 3 0x50 (u8_of_ireg r1) in
      if rex = 0x40 then
        Some [op]
      else 
        Some [rex, op] |
  ...
  Ppopl rd \<Rightarrow>
    ...
    let (op::u8) = bitfield_insert 0 3 0x58 (u8_of_ireg rd) in
      if rex = 0x40 then
        Some [op]
      else 
        Some [rex, op] |
 ...
 \<comment> \<open> P2892 `TEST:  qwordregister1 and qwordregister2`   -> ` 0100 1R0B 1000 0101 : 11 qwordreg1 qwordreg2` \<close>
  ...
  Pjcc t d \<Rightarrow>
    let (ex::u8) = 0x0f in
    let (op::u8) = bitfield_insert 0 4 0x80 (u8_of_cond t) in
...
value "x64_encode (Pleaq RBX (Addrmode None None 0))"*)
