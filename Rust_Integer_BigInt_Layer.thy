theory Rust_Integer_BigInt_Layer
  imports
    Rust_Base_Setup
...
text \<open>
  Internal signed-only layer for exports that deliberately retain Isabelle's
  binary natural-number datatype.  Normal clients should import
  \<^theory_text>\<open>Rust_BigInt_Setup\<close>, which selects integer, int, and nat
  together.
\<close>
