...
text \<open>
  ...
  This setup is an alternative to the BigInt integer setup; it
  deliberately does not import or modify that baseline setup.
\<close>
...
mod tests {
    ...
    fn bigint(value: RustInt) -> BigInt {
        value.into_bigint()
    }

    #[test]
    fn small_representation_has_a_bounded_layout() {
        // The i128 payload requires 16-byte alignment on this target.  Boxing
        // BigInt prevents the inactive Big variant from embedding its buffers
        // in every Small value.
        let size = size_of::<RustInt>();
        let align = align_of::<RustInt>();
        eprintln!("RustInt layout: size={size}, align={align}");
        assert!(size <= 32);
        assert!(align <= 16);
    }

    #[test]
    fn promotes_on_i128_overflow_and_demotes_afterwards() {
        let promoted = RustInt::Small(i128::MAX) + RustInt::ONE;
        assert!(matches!(promoted, RustInt::Big(_)));
        assert_eq!(
            bigint(promoted.clone()),
            BigInt::from(i128::MAX) + BigInt::from(1u8)
        );
        let demoted = promoted - RustInt::ONE;
        assert!(matches!(demoted, RustInt::Small(value) if value == i128::MAX));

        let promoted = RustInt::Small(i128::MIN) - RustInt::ONE;
        assert!(matches!(promoted, RustInt::Big(_)));
        let demoted = promoted + RustInt::ONE;
        assert!(matches!(demoted, RustInt::Small(value) if value == i128::MIN));
    }

    #[test]
    fn handles_i128_min_neg_abs_and_division_by_minus_one() {
        let magnitude = BigInt::from(1u8) << 127usize;
        assert_eq!(bigint(-RustInt::Small(i128::MIN)), magnitude);
        assert_eq!(bigint(abs(RustInt::Small(i128::MIN))), magnitude);
        assert_eq!(
            bigint(RustInt::Small(i128::MIN) / RustInt::Small(-1)),
            magnitude
        );
        assert_eq!(
            bigint(RustInt::Small(i128::MIN) % RustInt::Small(-1)),
    ...
    #[test]
    fn implements_isabelle_floor_division_in_all_sign_quadrants() {
        let cases = [
            (7, 3, 2, 1),
            (-7, 3, -3, 2),
            (7, -3, -3, -2),
        ...
        for (a, b, expected_q, expected_r) in cases {
            let (q, r) = div_mod_floor(RustInt::Small(a), RustInt::Small(b));
            assert_eq!(bigint(q), BigInt::from(expected_q));
            assert_eq!(bigint(r), BigInt::from(expected_r));
        }

        let (q, r) = div_mod_floor(RustInt::Small(-7), RustInt::ZERO);
        assert_eq!(bigint(q), BigInt::ZERO);
        assert_eq!(bigint(r), BigInt::from(-7));
    }

    #[test]
    fn preserves_infinite_twos_complement_bitwise_semantics() {
        assert_eq!(
            bigint(RustInt::Small(-1) & RustInt::Small(0x55)),
            BigInt::from(0x55)
        );
        assert_eq!(bigint(!RustInt::ZERO), BigInt::from(-1));

        let beyond_i128 = normalize(BigInt::from(1u8) << 127usize);
        assert_eq!(
            bigint(beyond_i128 | RustInt::ONE),
