...
/// Summary of target-aware BigInt bit-operation lowering.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct BigIntBitOperationsAnalysis {
...
/// `num_bigint::BigInt` to the corresponding native BigInt operators.
///
...
///
/// The evaluation in this paper does not exercise this standalone pass.
pub fn lower_bigint_bit_operations(module: &mut RustModule) -> BigIntBitOperationsAnalysis {
    let mut analysis = BigIntBitOperationsAnalysis::default();
    lower_module(module, &mut analysis);
    analysis
}

fn lower_module(module: &mut RustModule, analysis: &mut BigIntBitOperationsAnalysis) {
...
fn lower_impl(impl_block: &mut ImplBlock, analysis: &mut BigIntBitOperationsAnalysis) {
...
mod tests {
    ...
    fn optimize(source: &str) -> (BigIntBitOperationsAnalysis, String) {
        let mut module = parse_rust_source(source, "test").expect("source should parse");
        let analysis = lower_bigint_bit_operations(&mut module);
...
"#;
        ...
        assert_eq!(analysis, BigIntBitOperationsAnalysis::default());
